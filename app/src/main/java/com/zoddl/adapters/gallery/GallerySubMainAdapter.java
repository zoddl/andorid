...
public class GallerySubMainAdapter extends RecyclerView.Adapter<GallerySubMainAdapter.ViewHolder> implements StringConstant, OnPageChangeListener, OnLoadCompleteListener{
    private Context context;
    private List<ImagesItem> images;

    public GallerySubMainAdapter(Context context, List<ImagesItem> images) {
        ...
        try {
           ...
           /* if (images.get(i).getTagType().equalsIgnoreCase(_OTHER)) {
            ...
            for (int k = 0; k < images.get(i).getSecondaryTag().size(); k++) {
                SecondaryTag item = images.get(i).getSecondaryTag().get(k);
                if (k == 0)
            ...
            if (images.get(i).getIsUploaded().equalsIgnoreCase("0")) {
                holder.img_yellow.setBackgroundResource(R.drawable.circle_status_button_upload);
                ...
                holder.img_yellow.setBackgroundResource(R.drawable.circle_status_button);
                Glide.with(context)
                        .load(imageUrl)
                        .asBitmap()
